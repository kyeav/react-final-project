          ...
          {!loading ? (
            <div className="flex flex-col space-y-8 lg:space-y-0 lg:flex-row lg:items-start lg:space-x-12">
              ...
              <div className="flex flex-col flex-1">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 max-w-3xl gap-5 mb-8">
                  {director ? (
                    <MovieFact name="Director" value={director?.name} />
                  ) : (
                    <MovieFact name="Director" value="N/A" />
                  )}
                  {movie?.vote_average > 0 ? (
                    ...
                    <MovieFact name="User Score" value="N/A" />
                  )}
                  {movie?.budget > 0 ? (
                    <MovieFact
                      name="Budget"
                    ...
                    <MovieFact name="Budget" value="N/A" />
                  )}
                  {movie?.revenue > 0 ? (
                    <MovieFact
                      name="Box Office"
                    ...
                    <MovieFact name="Box Office" value="N/A" />
                  )}
                </div>
                {/* Favourites Button */}
                <button
                  onClick={() =>
                    inFavourites ? removeFromFavourites() : addToFavourites()
                  }
                  className={`self-start border-2 uppercase py-2 px-8 tracking-widest font-semibold duration-150 ease text-sm 
                    w-full sm:w-auto
                    ${
                      !inFavourites
                        ? "text-white border-[#68ded7] hover:bg-[#68ded7] hover:text-black"
            ...
            <div className="flex flex-col space-y-12 lg:space-y-0 lg:flex-row lg:items-start lg:space-x-12">
              ...
              <div className="flex flex-col flex-1">
                {/* Movie Title */}
                <div
                  className="text-2xl lg:text-5xl font-bold leading-tight mb-4
                  w-full animated-bg max-w-sm"
                >
                  &nbsp;
                </div>
                {/* Movie Details */}
                <div className="w-full max-w-sm h-6 mb-8 animated-bg"></div>
                {/* Movie Tagline */}
                ...
                <div className="h-24 max-w-3xl animated-bg"></div>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="relative container max-w-7xl mx-auto px-6 py-12 md:py-20">
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-6 gap-y-12">
              ...
              {!loading
                ...
                : new Array(12)
                    .fill(0)
                    .map((_, index) => <CastSkeleton key={index} />)}
            </div>
          </div>
        </section>
      )}
